...
def getConvNet(out_classes: int = 2, input_shape: tuple = (192, 256, 256, 1)) -> Model:
    ...
    # 192 x 256 x 256 x 1
    x = Conv3D(
        8, (3, 3, 3), padding="same", use_bias=False, kernel_initializer="he_normal"
    )(inp)
    ...
    # 96 x 128 x 128 x 8
    x = Conv3D(
        16, (3, 3, 3), padding="same", use_bias=False, kernel_initializer="he_normal"
    )(x)
    x = BatchNormalization(axis=4)(x)
    x = Activation("relu")(x)
    ...
    # 48 x 64 x 64 x 16
    x = Conv3D(
        32, (3, 3, 3), padding="same", use_bias=False, kernel_initializer="he_normal"
    )(x)
    x = BatchNormalization(axis=4)(x)
    x = Activation("relu")(x)
    ...
    # 24 x 32 x 32 x 32
    x = Conv3D(
        64, (3, 3, 3), padding="same", use_bias=False, kernel_initializer="he_normal"
    )(x)
    x = BatchNormalization(axis=4)(x)
    x = Activation("relu")(x)
    ...
    # 12 x 16 x 16 x 64
    x = Conv3D(
        128, (3, 3, 3), padding="same", use_bias=False, kernel_initializer="he_normal"
    )(x)
    x = BatchNormalization(axis=4)(x)
    x = Activation("relu")(x)
    ...
    x = Dropout(0.5)(
        x, training=True
    )  # = always apply dropout (permanent 'training mode')
