...
def test_compute_mean_prob_and_uncertainty():
    ...
    assert std.iloc[0] == np.std([0.6, 0.7, 0.8, 0.5, 0.6])
    assert mean.iloc[1] == np.mean([0.3, 0.2, 0.4, 0.3, 0.2])
    assert std.iloc[1] == np.std([0.3, 0.2, 0.4, 0.3, 0.2])
...
def test_merge_predictions_and_gt():
    ...
    pd.testing.assert_series_equal(result['mean_pred'], mean)
    pd.testing.assert_series_equal(result['std_pred'], std)
    pd.testing.assert_series_equal(result['bin_gt'], ground_truth['bin_gt'])
...
def test_calculate_DFFMR_AP_AUROC():
    merged = pd.DataFrame({'mean_pred': [0.7, 0.3, 0.8, 0.4],   
                            'std_pred': [0.2, 0.1, 0.3, 0.0], 
                            'scaled_std_pred': [0.667, 0.332, 1, 0], 
                            'bin_gt': [1, 0, 1, 0]})
    ...
    assert DFFMR == 1.0 # all images are flagged so none of them are retained
    assert np.isnan(AP)
    assert np.isnan(AUC)
